export const weddingPageData = {
  banner: {
    video: {
      src: "/wedding_hero.mp4",
      poster: "/wedding_hero.png",
    },
  },

  introData: {
    title: "A Grand Beginning to Your ForeveR!",
    description: [
      "Weddings become truly unforgettable when celebrated amidst breathtaking landscapes, pleasant weather, and lush green surroundings, and Corbett the Grand offers the perfect setting for a magical destination wedding. Spread across 20+ acres of beautifully landscaped gardens, the resort features 150+ spacious cottages and rooms, four scenic lawns, and a seamless blend of indoor and outdoor venues, including four air-conditioned, versatile banquet spaces opening out to the pool and gardens. With two beautiful swimming pools, three multi-cuisine restaurants, and three well-equipped conference halls, the property is designed to host celebrations of every scale, accommodating up to 2,000 guests with ease—making Corbett the Grand an ideal choice for an elegant, memorable destination wedding.",
    ],
    image: "/wedding/intro-1.png",
    video: {
      src: "https://eazotel-client-webp-image.s3.ap-south-1.amazonaws.com/corbatt-the-grand/Haldi-Reel-Corbett-grand.mp4",
      poster: "/wedding/poster.png",
    },
    link: {
      label: "CONTACT US",
      href: "/contact",
    },
  },

  highlightData: {
    items: [
      "150+ plush Cottages and Rooms",
      "2000 guests seated",
      "Classic Celebration",
      "Royal Affair",
    ],
  },

  venueData: {
    title: "The perfect choice for your destination wedding",
    description: [
      "At Corbett The Grand, each venue is infused with natural charm and elegance. With inbuilt modern technology, the latest theme-based décor, professional and friendly service, and abundant free parking available on site, Corbett The Grand stands out as one of the most preferred and popular choices for large ceremonies across the entire Corbett region.",
      "Corbett The Grand caters to a wide range of functions and events—be it a gala dinner, intimate gathering, wedding, or anniversary. The largest pillar-less hall, spanning 4,200 sq. ft., can comfortably accommodate up to 350 guests and features a dedicated stage and dance floor. Our beautifully landscaped gardens provide the perfect setting for Mehendi, Sagan ceremonies, and grand receptions."
    ],
    images: ["/wedding/venue-1.png", "/wedding/venue-2.png"],
    link: {
      label: "CONTACT US",
      href: "/contact",
    },
  },

  curatedData: {
    title: "Your Dream Wedding, Perfectly Curated",
    description: [
      "Every wedding should be as unique as your love. At Corbett The Grand, personalize every detail with curated packages and enjoy a seamless, all-in-one celebration—from stunning vows to elegant bridal suites.",
    ],
    images: ["/wedding/curated-1.png", "/wedding/curated-2.png"],
  },

  faqData: {
    title: "Frequently Asked Questions",
    img: "/wedding/faq.png",
    items: [
      {
        question:
          "What types of wedding functions can be hosted at Corbett The Grand?",
        answer:
          "We host all wedding-related celebrations including engagements, Mehendi, Sagan, Haldi, weddings, receptions, and post-wedding brunches.",
      },
      {
        question: "What is the guest accommodation capacity?",
        answer:
          "Corbett The Grand comfortably accommodates up to 150+ rooms for guests, making it ideal for destination weddings.",
      },
      {
        question: "What is the maximum capacity for wedding events?",
        answer:
          "Our venues can host up to 2,000 seated guests for grand celebrations, along with elegant indoor and outdoor options.",
      },
      {
        question: "Do you offer indoor and outdoor wedding venues?",
        answer:
          "Yes, we offer pillar-less banquet halls, beautifully landscaped lawns, and gardens for both indoor and outdoor ceremonies.",
      },
      {
        question: "Are customized wedding packages available?",
        answer:
          "Absolutely. Our wedding packages are fully customizable to match your theme, preferences, and budget.",
      },
      {
        question: "Do you provide decor and theme planning services?",
        answer:
          "Yes, we offer the latest theme-based décor and work closely with professional decorators to bring your vision to life.",
      },
      {
        question: "Is in-house catering available?",
        answer:
          "Yes, we offer a wide range of curated menus with multi-cuisine options tailored to your event.",
      },
      {
        question: "Is ample parking available for guests?",
        answer:
          "Yes, we provide abundant free parking on site for the convenience of your guests.",
      },
      {
        question: "Do you provide bridal suites?",
        answer:
          "Yes, we offer elegant bridal suites with dedicated beauty stations and spacious interiors for pre-ceremony preparations.",
      },
      {
        question: "Can the property be booked exclusively for weddings?",
        answer:
          "Exclusive buyouts can be arranged based on availability and event requirements.",
      },
    ],
  },
};
